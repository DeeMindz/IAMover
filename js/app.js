  ...
  if (e.data.type === 'IAM_CONV_CREATE') {
    ...
    try {
      ...
      if (data.conversation_id) {
        ...
        // widget polling handles delivery
      }
  ...
  if (e.data.type === 'IAM_BOT_REQUEST') {
    ...
    try {
      ...
      if (data.hitl_active) {
        ...
        console.log('[IAM Bridge] HITL active — telling widget to poll:', activeConvId);
        // postMessage to widget iframe — triggers polling for agent messages
...
function subscribeToConversation(conversationId) {
  ...
  // Appends directly to DOM so agent sees user messages in real-time.
  activeSubscription = Conversations.subscribeToMessages(conversationId, (msg) => {
    ...
    if (msg.role === 'system') {
      ...
      // Skip if already shown (optimistic UI)
...
// ── Dashboard HITL Polling ────────────────────────────────────────
// Polls for new messages every 2s during HITL so agent sees user
    ...
    try {
      ...
      if (msgsEl) {
        ...
        msgsEl.querySelectorAll('span').forEach(el => {
          ...
          if (t.includes('has resumed')) shown.add('__sys__agent_left');
        });
      }

      data.messages.forEach(m => {
        if (!_dashLastMsgAt || m.created_at > _dashLastMsgAt) _dashLastMsgAt = m.created_at;
        if (!msgsEl || AppState.activeConversation !== convId) return;
        if (shown.has(m.content.trim())) return;
        ...
        if (m.role === 'user') {
          div.className = 'message user';
          div.setAttribute('data-rawcontent', m.content);
          div.innerHTML = `<div class="msg-bubble">${m.content}</div><div class="msg-time">${time}</div>`;
        } else if (m.role === 'bot') {
          div.className = 'message bot';
          div.setAttribute('data-rawcontent', m.content);
          div.innerHTML = `<div class="msg-avatar" style="background:${botColor}33;color:${botColor};font-weight:700;font-size:12px;">${botInitial}</div><div><div class="msg-bubble">${m.content}</div><div class="msg-time">${time}</div></div>`;
        ...
        if (div.className || div.style.cssText) {
          msgsEl.appendChild(div);
          msgsEl.scrollTop = msgsEl.scrollHeight;
          shown.add(m.content.trim());
        }
      });

      // Stop if HITL ended
      if (data.hitl_active === false) stopDashboardPoll();
    } catch(e) { console.warn('[Dashboard Poll]', e); }
  }, 2000);
  console.log('[Dashboard] HITL polling started for conv:', convId);
}
...
async function interceptConversation() {
  ...
  // Immediately notify widget iframes — don't wait for user to send a message
  // This ensures the user sees "A live agent has joined" right away
  [document.getElementById('preview-iframe'), document.getElementById('preview-iframe-inline')]
    .filter(f => f && f.contentWindow)
    .forEach(f => f.contentWindow.postMessage({ type: 'IAM_HITL_ACTIVE', conv_id: convId }, '*'));
  ...
  // Start dashboard polling so agent sees user messages live
  startDashboardPoll(convId);
...
// Typing indicator — broadcasts to widget via agent_typing column
...
async function sendAgentMessage() {
  ...
  // Optimistic UI — show immediately without waiting for API
  const msgsEl = document.getElementById('conv-messages');
  if (msgsEl) {
    const div = document.createElement('div');
    div.className = 'message bot';
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .bot-wrap { display:flex; flex-direction:column; align-items:flex-start; max-width:82%; margin-bottom:16px; }
  ...
  .iam-fb-form button { background:#10b981; color:#fff; border:none; border-radius:6px; padding:6px; font-size:11px; font-weight:600; cursor:pointer; }
  /* Chat window */
  .chat-window {
    position:fixed; bottom:24px; right:24px;
...
<body>
  ...
  <div class="launcher" id="launcher" onclick="openChat()">
    ${ launcherUrl ? '<img src="' + launcherUrl + '" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" onerror="this.style.display=\'none\'" />' : '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" style="margin-top:2px;"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>' }
...
<script>
  ...
  function formatMarkdown(text) {
    ...
    // 1. Markdown links [text](url) -> <a> tags directly
    h = h.replace(/\\[([^\\]]+)\\]\\(([^)]+)\\)/g, '<a href="$2" target="_blank" rel="noopener">$1</a>');

    // 2. Auto-link bare URLs NOT already inside href="..."
  ...
  window._lastUserStatusEl = appendUser(text);
  input.value = '';
...
window.addEventListener('message', function (e) {
  ...
  if (e.data.type === 'IAM_CONV_CREATED') {
    ...
    // Start background status check — detects when agent takes over
    // even if user hasn't sent a message yet
    startStatusCheck(e.data.conv_id);
  }
...
// Runs every 5s during normal conversation to detect HITL takeover.
// When agent takes over, switches to fast 2s HITL polling automatically.
var _statusCheckInterval = null;

function startStatusCheck(cId) {
  stopStatusCheck();
  _statusCheckInterval = setInterval(function() {
    if (window._pollInterval) return; // already in HITL polling, skip
...
function startHITLPolling(cId) {
  ...
  console.log('[IAM Preview] HITL polling started:', cId);
}
...
function confirmNewConv() {
  ...
  stopHITLPolling(); stopStatusCheck(); window._previewConvId=null; window._lastMsgAt=null;
  var msgs=document.getElementById('chat-messages');
